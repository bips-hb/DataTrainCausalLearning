...
plot.age <- ggplot(bcrot, aes(x=age, after_stat(density), fill=as.factor(hormon))) +
    geom_histogram(color="#e9ecef", binwidth = 2) +
...
# Lymph nodes ~ hormon
plot.nodes <- ggplot(bcrot, aes(x = nodes, y = after_stat(density), fill = as.factor(hormon))) +
  geom_histogram(color = "#e9ecef", binwidth = 1) +
  facet_grid(as.factor(hormon) ~ .) +
  labs(
...
ggplot(bcrot, aes(x=enodes, after_stat(density), fill=as.factor(hormon))) +
  geom_histogram(color=c("#e9ecef"), binwidth = 0.1) +
  facet_grid(as.factor(hormon) ~ .) +
...
ggplot(bcrot, aes(x=pr_1, after_stat(density), fill=as.factor(hormon))) +
  geom_histogram(color="#e9ecef", binwidth = 0.5) +
  facet_grid(as.factor(hormon) ~ .) +
